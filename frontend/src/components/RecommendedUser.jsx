...
const RecommendedUser = ({ user }) => {
  ...
  console.log(connectionStatus?.data.content.status);
  const renderButton = () => {
    if (isLoading) {
      return (
        <button
    ...
    switch (connectionStatus?.data?.content?.status) {
      case "pending":
        return (
          <button
      ...
      case "received":
        return (
          <div className="flex gap-2 justify-center">
            <button
              onClick={() =>
                acceptRequest(connectionStatus.data.content.requestId)
              }
            ...
            <button
              onClick={() =>
                rejectRequest(connectionStatus.data.content.requestId)
              }
  ...
  const handleConnect = () => {
    if (connectionStatus?.data?.content?.status === "not_connected") {
